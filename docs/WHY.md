...
```tsx
useDataSync(() => {
  refreshPreview(); // Fires when the server announces its data changed
});
```

One hook. No polling. The server announces its own changes with the spec's `notifications/resources/list_changed`, and the host forwards it to the iframe — whether the agent, a webhook, or another view made the change.
