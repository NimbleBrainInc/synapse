...
extension registered aborts the server at connect time. `mode="optional"` lets
the same tool run inline (`callTool`) or as a task (`callToolAsTask`), so the
client decides per call. `mode="required"` rejects non-task calls with JSON-RPC
`-32601`.
