...
- **The `@modelcontextprotocol/ext-apps` peer range is `^1.7.5`, and the host's tasks capability is read from `hostCapabilities.experimental`.** `connect()` reads one key — `experimental["io.modelcontextprotocol/tasks"]`, the identifier the MCP extension registry defines for Tasks. A top-level `hostCapabilities.tasks` is no longer read, and no vendor alias is read either.
  ...
  **Migration:** install `@modelcontextprotocol/ext-apps@^1.7.5`, and publish the capability as `experimental: { "io.modelcontextprotocol/tasks": { cancel: {}, requests: { tools: { call: {} } } } }`. A host that advertises only a top-level `tasks`, or the same capability under another name, now reads as not supporting tasks: `app.supportsTasks` is `false` and `callToolAsTask` throws.
