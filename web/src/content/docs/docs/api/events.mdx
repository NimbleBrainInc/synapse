...
Any wire method works as a passthrough event name. For example,
`app.on("notifications/resources/list_changed", ...)` fires when your server
announces its data changed, which is what [`useDataSync`](/docs/guides/keep-ui-in-sync)
listens for.
